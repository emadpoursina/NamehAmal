...
import { resolveActivityLabel } from "@/app/lib/pomodoro/desktop-connection";
...
export function PomodoroView() {
  ...
  // Waiting for the desktop host (FR-009, data-model.md §4): render a neutral
  // placeholder — no fabricated countdown digits and no default durations.
  // Controls stay disabled; the real state arrives with the first host snapshot.
  if (connection === "waiting") {
    return (
      <div className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-black">
        <div className="flex flex-col items-center gap-3 text-center">
          <div className="text-sm font-medium uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
            Waiting for desktop host…
          </div>
          <p className="max-w-sm text-sm text-zinc-600 dark:text-zinc-400">
            The pomodoro clock is owned by the menu bar. Controls unlock once
            the desktop host connects.
          </p>
          <div className="mt-2 flex flex-wrap items-center justify-center gap-2">
            ...
            <button
              type="button"
              disabled
              className="rounded-md border border-zinc-300 px-4 py-2 text-sm font-medium text-zinc-800 disabled:cursor-not-allowed disabled:opacity-50 dark:border-zinc-700 dark:text-zinc-200"
            >
              Stop
            </button>
            <button
        ...
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          {connection === "connected" ? (
            <p
              ...
              data-testid="pomodoro-activity-label"
            >
              Activity: {resolveActivityLabel(activityName)}
            </p>
          ) : null}
