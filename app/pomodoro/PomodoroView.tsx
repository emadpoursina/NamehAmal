...
export function PomodoroView() {
  const { state, start, stop, skip, updateSettings, setWidgetHidden } = usePomodoro();
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-black">
        ...
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          The compact timer stays visible on other pages unless you hide it.
        </p>
        <button
          type="button"
          onClick={() => setWidgetHidden(!state.widgetHidden)}
          className="mt-3 rounded-md border border-zinc-300 px-4 py-2 text-sm font-medium text-zinc-800 transition-colors hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-950"
        >
          {state.widgetHidden ? "Show floating widget" : "Hide floating widget"}
        </button>
      </div>
